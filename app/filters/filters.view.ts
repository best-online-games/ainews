namespace $.$$ {
	export class $ainews_app_filters extends $.$ainews_app_filters {
		

		@$mol_mem
		include_string_value(next?:any){
			if(next !== undefined)
				return $mol_state_local.value("include_string_value", next) as any;
			return $mol_state_local.value("include_string_value") ?? ""
		}
		@$mol_mem
		exclude_string_value(next?:any){
			if(next !== undefined)
				return $mol_state_local.value("exclude_string_value", next) as any;
